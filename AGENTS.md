...
- **Overlay window** — `alwaysOnTop: 'floating'`, `visibleOnAllWorkspaces: true` so it follows the cursor across macOS Spaces; `setIgnoreMouseEvents(true)` so it never steals focus
