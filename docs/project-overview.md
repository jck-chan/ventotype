...
- **Typing** — on macOS the transcript is posted as synthetic key events (`native/typer.swift`, loaded with koffi like the fn tap), so the clipboard is never involved. A key event can carry a unicode string in place of a keycode, which is what makes arbitrary text possible; it goes out in runs of 20 UTF-16 units because the system truncates long ones, with flags cleared so a modifier still held from the shortcut can't alter the text. Windows and Linux keep the clipboard-and-paste route, the only one available there
